...
2. Write in to the object the key-value pairs for `name`, `email`,
 `age`, and `purchased`. Set the value of `purchased` to an empty array`[]`.
...
1. Our user has changed his or her email address. Without changing the
...
var updateUser = user.email 
var updateUser = user.age++
...
1. Without changing the original `user` object, add a new key `location` 
...
If we want to give our user a `friend` with a `name` and `age`, we could
 write:
...
When we console.log `user`, we would see the `friend` object added to
 our user object.
*/
...
/* 5. The `friend` has purchased "The One Ring". Use `.push()` to add 
"The One Ring" to the friend's `purchased` array. */
// ~~~~~~~ your solution here: ~~~~~~~~~~~~~
user.friend.purchased.push('The One Ring');


/* 6. The `friend` has purchased "A latte". Use `.push()` to add "A latte"
...
/* 1. Write a _for loop_ that iterates over the User's `purchased` 
array (NOT the friend's purchased array), and prints each element to 
the console. */
...
/* 2. Write a _for loop_ that iterates over the Friend's `purchased` 
array, and prints each element to the console. */

// ~~~~~~~ your solution here: ~~~~~~~~~~~~~

for(var i = 0; i<user.length; i++){
  console.log(user[i].friend.purchased);
}
...
When the function is run, it should:
  ...
  The function does not need a `return` statement, it will merely 
...
/* 2. Write a function `oldAndLoud` that performs the exact same 
tasks as `updateUser`, but instead of hard-coding it to only work on 
our `user` object, 
make it take a parameter `person`, and have it modify the object that 
is passed in as an argument when the function is called. Call your 
`oldAndLoud` 
