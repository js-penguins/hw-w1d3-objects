...
2. Write in to the object the key-value pairs for `name`, `email`, `age`, 
...
user.email="ameerakhuzaim2@gmail.com";
...
1. Without changing the original `user` object, add a new key `location` to the 
object, and give it a value or some-or-other location (a string).
*/

// ~~~~~~~ your solution here: ~~~~~~~~~~~~~

user.location="Riyadh, Badiah discrete";
//console.log(user);
...
Remember that you can add an object to an existing object in the same way that you 
can add any new property/value pair.
...
user.friend = {
  name: "Hessa Abdullah",
  age: 28,
  location:"Riyadh, Yasmin discrete",
  purchased:[]
}
//console.log(user);
...
/* 5. The `friend` has purchased "The One Ring". Use `.push()` to add "The One 
Ring" to the friend's `purchased` array. */
// ~~~~~~~ your solution here: ~~~~~~~~~~~~~
user.friend.purchased.push("The One Ring");
//console.log(user.friend.purchased);

/* 6. The `friend` has purchased "A latte". Use `.push()` to add "A latte" to the 
...
/* 1. Write a _for loop_ that iterates over the User's `purchased` array (NOT the 
...
/* 2. Write a _for loop_ that iterates over the Friend's `purchased` array, and 
...
/* 1. Write a single function `updateUser` that takes no parameters. When the 
function is run, it should:

  - it should increment the user's age by 1
  - make the user's name uppercase

  The function does not need a `return` statement, it will merely modify the user 
  object. */
...
/* 2. Write a function `oldAndLoud` that performs the exact same tasks as 
...
make it take a parameter `person`, and have it modify the object that is passed in
 as an argument when the function is called. Call your `oldAndLoud` 
